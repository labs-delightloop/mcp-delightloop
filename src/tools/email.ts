...
const EmailItemSchema = z.object({
  ...
  from: z
    ...
    .optional()
    .describe(
      "Sender email address. If omitted, auto-generated from your organization name " +
...
export const EmailBulkSendSchema = z.object({
  emails: z
    ...
    .min(1)
    .describe(
      "List of personalized emails to send. Each email can have its own recipient, " +
...
export async function emailSend(
  input: z.infer<typeof EmailSendSchema>,
  apiKey: string,
) {
  return dlRequest({
    method: "POST",
...
export async function emailBulkSend(
  input: z.infer<typeof EmailBulkSendSchema>,
  apiKey: string,
) {
  return dlRequest({
    method: "POST",
