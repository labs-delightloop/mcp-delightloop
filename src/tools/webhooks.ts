...
export const WebhookCreateSchema = z.object({
  ...
  events: z
    ...
    .min(1)
    .describe(
      "One or more event types to subscribe to. Available: " +
        WEBHOOK_EVENTS.join(", "),
    ),
});
...
export async function webhookGet(
  input: z.infer<typeof WebhookGetSchema>,
  apiKey: string,
) {
  return dlRequest({
    method: "GET",
...
export async function webhookList(
  input: z.infer<typeof WebhookListSchema>,
  apiKey: string,
) {
  return dlRequest({
    method: "GET",
...
export async function webhookDelete(
  input: z.infer<typeof WebhookDeleteSchema>,
  apiKey: string,
) {
  return dlRequest({
    method: "DELETE",
