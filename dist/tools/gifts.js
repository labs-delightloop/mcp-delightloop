...
export async function giftList(input, apiKey) {
    const query = {
        page: input.page ?? 1,
    ...
    if (input.collectionId)
        query.collectionId = input.collectionId;
    if (input.filterType)
        query.filterType = input.filterType;
...
export async function giftGet(input, apiKey) {
    return dlRequest({
        method: "GET",
