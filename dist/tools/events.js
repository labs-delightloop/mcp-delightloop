...
export const EventCreateSchema = z.object({
    ...
    type: z
        .string()
        .describe("Event type: 'conference', 'webinar', 'workshop', 'meetup', 'roadshow', or any string the org uses"),
    startDate: z
        .string()
        .describe("ISO date/datetime when the event starts (e.g. 2026-06-15 or 2026-06-15T09:00:00Z)"),
...
// ─── Tool handlers ────────────────────────────────────────────────────────────
export async function eventList(input, apiKey) {
...
export async function eventGet(input, apiKey) {
    const res = await dlRequest({
        method: "GET",
...
export async function eventCreate(input, apiKey) {
    const body = {
        name: input.name,
        type: input.type,
        eventDates: {
